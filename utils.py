...
def load_images(img_names, model_size):
    ...
    """
    imgs = []
#     fileTemp = open('./detections/test.txt', 'a')
                    
                    

    for img_name in img_names:
        img = Image.open(img_name)
#         tempstrTemp = 'Pass img = Image.open(img_name)\n'
#         fileTemp.writelines(tempstrTemp)
#         print('img shape = ' + str(np.array(img).shape))
        
        img = img.resize(size=model_size)
#         tempstrTemp = 'Pass img = img.resize(size=model_size)\n'
#         fileTemp.writelines(tempstrTemp)

        img = img.convert('RGB')
        
        img = np.array(img, dtype=np.float32)
#         tempstrTemp = 'Pass img = np.array(img, dtype=np.float32)\n'
#         fileTemp.writelines(tempstrTemp)

#         print(img_name)
#         print('img shape = ' + str(img.shape))
#         print('img dim = ' + str(img.ndim))
        
        img = np.expand_dims(img[:, :, :3], axis=0)
#         print('--------------After np expand dims------------')
#         print('img shape = ' + str(img.shape))
#         print('img dim = ' + str(img.ndim))
        imgs.append(img)
...
def save_detections(cat, img_names, boxes_dicts, class_names):
